...
public class PhysicsSystem implements SubSystem {
    ...
    public void processOneGameTick(EntityManager entityManager, double lastFrameTime) {
        ...
        for (Map.Entry<UUID, Velocity> velocityEntity : entityVelocityMap.entrySet()) {
            ...
            //update position
            if (position != null) {
                position.setX(position.getX() + (velocity.getMagnitude() * Math.cos(Math.toRadians(velocity.getDirection()))));
                position.setY(position.getY() + (velocity.getMagnitude() * Math.sin(Math.toRadians(velocity.getDirection()))));
            } else {
                try {
